...
match_phase3 <- function(data, emission_var) {
  ...
  match_out <- matchit(
   ...
   # distance = "mahalanobis",
      ratio = 2,
    #       replace = TRUE,
    caliper = c(Growth=0.3,TNitrogen_water=0.3),
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val,-Sector,-Subsector,-ETS,-TNitrogen_water,-Region)
  ...
  pretrends_plot <- ggplot(pretrend_plot_data, aes(x = year, y = mean_emission_log, color = factor(ETS))) +
    ...
    labs(title = paste("Time Trends -", "Total Nitrogen", "- Phase 3 Match"),
         x = "Year",
...
save(matched_data, file = "matched_data.TNitrogen_water.RData")
...
ggsave("TimeTrendTNitrogen_waterPhase3.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
mapshot2(
  Geography,
  file = "TNitrogen_waterGeoPhase3.png",
...
ggsave("LovePlotTNitrogen_waterPhase3.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
