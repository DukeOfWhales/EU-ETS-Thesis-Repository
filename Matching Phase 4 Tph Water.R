...
match_phase4 <- function(data, emission_var) {
  ...
  Firm_pretrend <- data_phase4 %>%
    ...
    summarize(
      ...
      Tph_Water = mean(emission_log, na.rm = TRUE),
      Region = first(Region),
      Sector = first(Sector),
  ...
  Firm_pretrend <- Firm_pretrend %>%
    filter(!is.na(Tph_Water), !is.na(Growth))
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val, -Region,-Sector,-Subsector,-ETS,-Tph_Water)
  ...
  pretrends_plot <- ggplot(pretrend_plot_data, aes(x = year, y = mean_emission_log, color = factor(ETS))) +
    ...
    labs(title = paste("Time Trends -", "Total Phosphorus", "- Phase 4 Match"),
         x = "Year",
...
matched_firms <- results_list[["Tph_Water"]]$matched_firms
matched_data <- results_list[["Tph_Water"]]$matched_data
match_out <- results_list[["Tph_Water"]]$match_out
...
save(matched_data, file = "matched_data4.Tph_Water.RData")
...
ggsave("TimeTrendTph_WaterPhase4.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotTph_WaterPhase4.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
