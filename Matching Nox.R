...
match_phase3 <- function(data, emission_var) {
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val,-Sector,-Subsector,-ETS,-Nox,-Region)
...
save(matched_data, file = "matched_data.Nox.RData")
...
ggsave("TimeTrendNoxPhase3.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotNoxPhase3.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
