...
match_phase3 <- function(data, emission_var) {
  ...
  match_out <- matchit(
    ...
    exact = ~ Sector +Region#+ Subsector + Region
  )
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val,-Sector,-Subsector,-ETS,-Nickel_Water,-Region)
...
save(matched_data, file = "matched_data.Nickel_Water.RData")
...
ggsave("TimeTrendNickel_WaterPhase3.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
mapshot2(
  Geography,
  file = "Nickel_WaterGeoPhase3.png",
...
ggsave("LovePlotNickel_WaterPhase3.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
