...
match_phase3 <- function(data, emission_var) {
  ...
  match_out <- matchit(
    ...
    ratio = 4,
    #    replace = TRUE,
      caliper = c(Growth=0.3,Tph_Water=0.3),
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val,-Sector,-Subsector,-ETS,-Tph_Water,-Region)
  ...
  pretrends_plot <- ggplot(pretrend_plot_data, aes(x = year, y = mean_emission_log, color = factor(ETS))) +
    ...
    labs(title = paste("Time Trends -", "Total Phosphorus", "- Phase 3 Match"),
         x = "Year",
...
# 1. Create a list to store results for each variable using pipeline
...
save(matched_data, file = "matched_data.Tph_Water.RData")
...
ggsave("TimeTrendTph_WaterPhase3.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotTph_WaterPhase3.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
