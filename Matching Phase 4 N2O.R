...
#    ungroup()
  ...
  Firm_pretrend <- data_phase4 %>%
    ...
    summarize(
      ...
      N2o = mean(emission_log, na.rm = TRUE),
      Region = first(Region),
      Sector = first(Sector),
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val, -Region,-Sector,-Subsector,-ETS,-N2o)
...
matched_firms <- results_list[["N2o"]]$matched_firms
matched_data <- results_list[["N2o"]]$matched_data
match_out <- results_list[["N2o"]]$match_out
...
save(matched_data, file = "matched_data4.N2o.RData")
...
ggsave("TimeTrendN2oPhase4.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotN2oPhase4.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
