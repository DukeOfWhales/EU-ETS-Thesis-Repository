...
match_phase3 <- function(data, emission_var) {
  ...
  match_out <- matchit(
    ...
    distance = "mahalanobis",
      ratio = 2,
    #    replace = TRUE,
    #  caliper = c(Growth=0.3,Nh3=0.3),
    #  min.controls = 1, max.controls = 5,
      exact = ~ Sector + Region #+ Subsector
  )
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val,-Sector,-Subsector,-ETS,-Nh3,-Region)
...
save(matched_data, file = "matched_data.Nh3.RData")
...
ggsave("TimeTrendNh3Phase3.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotNh3Phase3.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
