...
match_phase3 <- function(data, emission_var) {
  ...
  match_out <- matchit(
    ...
    ratio = 2,
    #    replace = TRUE,
  #  caliper = c(Growth=0.3,Toc_Water=0.3),
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val,-Sector,-Subsector,-ETS,-Toc_Water,-Region)
  ...
  pretrends_plot <- ggplot(pretrend_plot_data, aes(x = year, y = mean_emission_log, color = factor(ETS))) +
    ...
    labs(title = paste("Time Trends -", "TOC", "- Phase 3 Match"),
         x = "Year",
...
save(matched_data, file = "matched_data.Toc_Water.RData")
...
ggsave("TimeTrendToc_WaterPhase3.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotToc_WaterPhase3.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
