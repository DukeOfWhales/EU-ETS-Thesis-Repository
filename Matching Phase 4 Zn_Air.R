...
match_phase4 <- function(data, emission_var) {
  ...
  Firm_pretrend <- data_phase4 %>%
    ...
    summarize(
      ...
      Zn_Air = mean(emission_log, na.rm = TRUE),
      Region = first(Region),
      Sector = first(Sector),
  ...
  Firm_pretrend <- Firm_pretrend %>%
    filter(!is.na(Zn_Air), !is.na(Growth))
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val, -Region,-Sector,-Subsector,-ETS,-Zn_Air)
...
matched_firms <- results_list[["Zn_Air"]]$matched_firms
matched_data <- results_list[["Zn_Air"]]$matched_data
match_out <- results_list[["Zn_Air"]]$match_out
...
save(matched_data, file = "matched_data4.Zn_Air.RData")
...
ggsave("TimeTrendZn_AirPhase4.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotZn_AirPhase4.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
