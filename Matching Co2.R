...
match_phase3 <- function(data, emission_var) {
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val,-Sector,-Subsector,-ETS,-Co2,-Growth,-Region)
...
Firm_pretrend <- results_list[["Co2"]]$Firm_pretrend
...
# Make a map of facility locations using the matched data Lat and Lon

# 13. Save the matched data
save(matched_data, file = "matched_data.Co2.RData")
...
ggsave("TimeTrendCo2Phase3.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotCo2Phase3.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
...
matched_data2 <- matched_data %>%
  mutate(
    Region = factor(Region)
  )

sum_w  = sum(matched_data$weights, na.rm = TRUE)
n_firm = n_distinct(matched_data$Firm)
