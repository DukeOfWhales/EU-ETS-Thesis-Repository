...
match_phase3 <- function(data, emission_var) {
  ...
  #  ratio = 2,
 #       replace = TRUE,
   # caliper = c(Growth=0.3,Zn_Air=0.3),
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val,-Sector,-Subsector,-ETS,-Zn_Air,-Region)
...
save(matched_data, file = "matched_data.Zn_Air.RData")
...
ggsave("TimeTrendZn_AirPhase3.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotZn_AirPhase3.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
