...
match_phase4 <- function(data, emission_var) {
  ...
  data_phase4 <- data_phase4 %>%
    ...
    filter( sum(year >= 2013 & year <= 2020, na.rm = TRUE) >= 4 ) %>%
    ungroup()
  ...
  Firm_pretrend <- data_phase4 %>%
    ...
    summarize(
      ...
      Nox = mean(emission_log, na.rm = TRUE),
      Region = first(Region),
      Sector = first(Sector),
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val, -Region,-Sector,-Subsector,-ETS,-Nox)
...
matched_firms <- results_list[["Nox"]]$matched_firms
matched_data <- results_list[["Nox"]]$matched_data
match_out <- results_list[["Nox"]]$match_out
...
save(matched_data, file = "matched_data4.Nox.RData")
...
ggsave("TimeTrendNoxPhase4.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotNoxPhase4.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
