...
match_phase4 <- function(data, emission_var) {
  ...
  Firm_pretrend <- data_phase4 %>%
    ...
    summarize(
      ...
      Co2ExclBio = mean(emission_log, na.rm = TRUE),
      Region = first(Region),
      Sector = first(Sector),
  ...
  Firm_pretrend <- Firm_pretrend %>%
    filter(!is.na(Co2ExclBio), !is.na(Growth))
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val, -Region,-Sector,-Subsector,-ETS,-Co2ExclBio)
...
matched_firms <- results_list[["Co2ExclBio"]]$matched_firms
matched_data <- results_list[["Co2ExclBio"]]$matched_data
match_out <- results_list[["Co2ExclBio"]]$match_out
...
save(matched_data, file = "matched_data4.Co2ExclBio.RData")
...
ggsave("TimeTrendCo2ExclBioPhase4.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotCo2ExclBioPhase4.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
