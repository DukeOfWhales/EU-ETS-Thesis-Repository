...
match_phase4 <- function(data, emission_var) {
  ...
  Firm_pretrend <- data_phase4 %>%
    ...
    summarize(
      ...
      Nickel_Water = mean(emission_log, na.rm = TRUE),
      Region = first(Region),
      Sector = first(Sector),
  ...
  Firm_pretrend <- Firm_pretrend %>%
    filter(!is.na(Nickel_Water), !is.na(Growth))
  ...
  match_out <- matchit(
    ETS ~ Nickel_Water + Growth,
    data = Firm_pretrend,
    method = "nearest",
    distance = "mahalanobis",
      ratio = 2,
    #    replace = TRUE,
    #  caliper = c(Growth = 0.3, Nickel_Water = 0.3),
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val, -Region,-Sector,-Subsector,-ETS,-Nickel_Water)
...
matched_firms <- results_list[["Nickel_Water"]]$matched_firms
matched_data <- results_list[["Nickel_Water"]]$matched_data
match_out <- results_list[["Nickel_Water"]]$match_out
...
save(matched_data, file = "matched_data4.Nickel_Water.RData")
...
ggsave("TimeTrendNickel_WaterPhase4.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
mapshot2(
  Geography,
  file = "Nickel_WaterGeoPhase4.png",
...
ggsave("LovePlotNickel_WaterPhase4.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
