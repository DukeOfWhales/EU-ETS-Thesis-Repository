...
match_phase4 <- function(data, emission_var) {
  ...
  Firm_pretrend <- data_phase4 %>%
    ...
    summarize(
      ...
      Nickel_Air = mean(emission_log, na.rm = TRUE),
      Region = first(Region),
      Sector = first(Sector),
  ...
  Firm_pretrend <- Firm_pretrend %>%
    filter(!is.na(Nickel_Air), !is.na(Growth))
  ...
  match_out <- matchit(
    ETS ~ Nickel_Air + Growth,
    data = Firm_pretrend,
    method = "nearest",
  #  distance = "mahalanobis",
#    ratio = 2,
    #    replace = TRUE,
    #  caliper = c(Growth = 0.3, Nickel_Air = 0.3),
    #  min.controls = 1, max.controls = 10,
    exact = ~ Sector #+ Region  
  )
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val, -Region,-Sector,-Subsector,-ETS,-Nickel_Air)
  ...
  pretrends_plot <- ggplot(pretrend_plot_data, aes(x = year, y = mean_emission_log, color = factor(ETS))) +
    ...
    labs(title = paste("Time Trends -", emission_var, "- Phase 4 Match"),
         x = "Year",
...
matched_firms <- results_list[["Nickel_Air"]]$matched_firms
matched_data <- results_list[["Nickel_Air"]]$matched_data
match_out <- results_list[["Nickel_Air"]]$match_out
...
save(matched_data, file = "matched_data4.Nickel_Air.RData")
...
ggsave("TimeTrendNickel_AirPhase4.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotNickel_AirPhase4.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
