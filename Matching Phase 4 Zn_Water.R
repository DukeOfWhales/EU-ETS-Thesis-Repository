...
# Count the number of unique firms for each year
unique_firms_per_year <- merged_data %>%
  group_by(year) %>%
  summarize(unique_firms = n_distinct(Firm), .groups = "drop")
...
match_phase4 <- function(data, emission_var) {
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val, -Region,-Sector,-Subsector,-ETS,-Zn_Water)
...
save(matched_data, file = "matched_data4.Zn_Water.RData")
...
ggsave("TimeTrendZn_WaterPhase4.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotZn_WaterPhase4.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
