...
match_phase4 <- function(data, emission_var) {
  ...
  Firm_pretrend <- data_phase4 %>%
    ...
    summarize(
      ...
      Ch4 = mean(emission_log, na.rm = TRUE),
      Region = first(Region),
      Sector = first(Sector),
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val, -Region,-Sector,-Subsector,-ETS,-Ch4)
...
matched_firms <- results_list[["Ch4"]]$matched_firms
matched_data <- results_list[["Ch4"]]$matched_data
match_out <- results_list[["Ch4"]]$match_out
...
plot(match_out, type = "ecdf", interactive = FALSE,
     which.xs = ~Ch4 + Growth)
#Love Plot#
plot(Match_summary,var.order = "alphabetical",abs = TRUE)
...
save(matched_data, file = "matched_data4.Ch4.RData")
...
ggsave("TimeTrendCh4Phase4.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotCh4Phase4.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
