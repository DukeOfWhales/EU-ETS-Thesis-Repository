...
match_phase4 <- function(data, emission_var) {
  ...
  Firm_pretrend <- data_phase4 %>%
    ...
    summarize(
      ...
      Sox = mean(emission_log, na.rm = TRUE),
      Region = first(Region),
      Sector = first(Sector),
  ...
  matched_firms <- matched_firms %>%
    select(-first_year, -last_year, -first_val, -last_val, -Region,-Sector,-Subsector,-ETS,-Sox)
...
matched_firms <- results_list[["Sox"]]$matched_firms
matched_data <- results_list[["Sox"]]$matched_data
match_out <- results_list[["Sox"]]$match_out
...
save(matched_data, file = "matched_data4.Sox.RData")
...
ggsave("TimeTrendSoxPhase4.png", plot = results_list[[pollutant_var]]$pretrends_plot, width = 5, height = 4, bg = "white")
...
ggsave("LovePlotSoxPhase4.png", plot = love.plot(
  match_out_relevant,
  abs        = TRUE,
  poly       = 2,
  binary     = "std",
